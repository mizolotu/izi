...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--destination', help='Server name', default='server2')
    parser.add_argument('-p', '--protocol', help='Protocol', default='tcp')
    parser.add_argument('-a', '--application', help='Application port', default=80, type=int)
    parser.add_argument('-l', '--label', help='DSCP label', default=0, type=int)
    ...
    ip_s_nat = nat_ip(ip_s, prefix='1{0}'.format(str(args.ids)))
    ip_s_nat_gw = gateway(ip_s_nat)
    ip_d_nat = nat_ip(ip_d, prefix='1{0}'.format(str(args.ids)))
    ...
    if container_s['vm'] in nodes.keys() and container_d['vm'] in nodes.keys():
        ...
        controller.ip_src_dst_port_mod_src_dst_output(nodes[container_s['vm']], ip_ids_table, high_priority, ip_s, ip_d, proto, dport, ip_s_nat, ip_d_nat, tunnel_to_ids)
        ...
        assert len(tunnel_to_mon) == 1
        tunnel_to_mon = tunnel_to_mon[0]
        controller.ip_src_dst_port_mod_src_dst_output(nodes[container_d['vm']], ip_ids_table, high_priority, ip_d, ip_s, proto, sport, ip_d_nat, ip_s_nat, tunnel_to_ids)
        ...
        assert len(tunnel_to_mon) == 1
        tunnel_to_mon = tunnel_to_mon[0]
        controller.ip_src_dst_port_mod_src_dst_output(nodes['gw'], ip_ids_table, high_priority, ip_d, ip_s, proto, sport, ip_d_nat, ip_s_nat, tunnel_to_ids)
        ...
        assert len(tunnel_to_mon) == 1
        tunnel_to_mon = tunnel_to_mon[0]
        controller.ip_src_dst_port_mod_src_dst_output(nodes['gw'], ip_ids_table, high_priority, ip_s, ip_d, proto, dport, ip_s_nat, ip_d_nat, tunnel_to_ids)
