...
from reinforcement_learning.common.vec_env.base_vec_env import VecEnv, CloudpickleWrapper
    ...
    while True:
        try:
            ...
            if cmd == 'step':
                ...
                rewards, infos = env.reward()
                remote.send((rewards, infos))
            elif cmd == 'close':
                env.close()
                remote.close()
                break
...
class SubprocVecEnv(VecEnv):
    ...
    def reward_one(self, env_idx):
        ...
        rewards, infos = self.remotes[env_idx].recv()
    ...
    def reset(self):
        ...
        if np.any([item is None for item in obs]):
            obs = None
        else:
