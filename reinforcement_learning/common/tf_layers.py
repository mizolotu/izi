...
def mlpstack(input_tensor, mask_tensor, state_hidden, scope, n_hidden, init_scale=1.0, layer_norm=False, act_fun=tf.tanh):
    ...
    for idx, (_input, mask) in enumerate(zip(input_tensor, mask_tensor)):
        ...
        hidden = act_fun(tf.matmul(cell_state, weight_x) + bias_x)
        output.append(act_fun(tf.matmul(hidden, weight_h) + bias_h))
